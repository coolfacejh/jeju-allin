...
try {
  for (const width of [390,1440]) {
    ...
    await p.getByRole('button',{name:'이 구간만 담기'}).click();
    assert.ok((await p.getByRole('status',{name:'구간 저장 결과'}).innerText()).includes('1~1440분'));
    await p.getByLabel('구간 방문 날짜').selectOption('2');
    await p.getByLabel('구간 계획 시간').fill('120');
    await p.getByRole('button',{name:'이 구간만 담기'}).click();
    assert.ok((await p.getByRole('status',{name:'구간 저장 결과'}).innerText()).includes('3일차에 일부 구간만 120분'));
    const segmentId=900000010010;
    trip=await p.evaluate(() => JSON.parse(localStorage.getItem('jeju_trip_v2')));
    assert.deepEqual(trip.days[2],[segmentId]);
    assert.ok(!trip.days.flat().includes(900000000010));
    await p.goto(origin+'/#/place/'+segmentId);
    await p.waitForURL('**/#/olle/01?segment=1');
    assert.equal(await p.getByLabel('구간 계획 시간').inputValue(),'120');
    await p.getByRole('button',{name:'담은 구간 일정 업데이트'}).click();
    trip=await p.evaluate(() => JSON.parse(localStorage.getItem('jeju_trip_v2')));
    assert.deepEqual(trip.days[2],[segmentId]);
    await p.screenshot({path:`output/verification/olle-access-${width}.png`,fullPage:false});
